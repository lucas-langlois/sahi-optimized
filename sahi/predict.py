...
def get_batch_prediction(
    ...
    Arguments:
        image_list: List of images (str paths or np.ndarray)
            List of images to process in batch
        detection_model: model.DetectionModel
            Detection model to use for inference
...
def get_sliced_prediction(
    ...
    num_group = int(np.ceil(num_slices / num_batch))
    if verbose == 1 or verbose == 2:
        tqdm.write(f"Performing prediction on {num_slices} slices with batch_size={num_batch}.")
    ...
    for group_ind in range(num_group):
        ...
        if batch_size_actual == 1 and num_batch == 1:
            ...
            try:
                batch_prediction_result = get_batch_prediction(
