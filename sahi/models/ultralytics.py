...
class UltralyticsDetectionModel(DetectionModel):
    ...
    def perform_batch_inference(self, image_list: List[np.ndarray]):
        """
        Batch prediction is performed using self.model and the prediction results are set to self._original_predictions.
        Args:
        ...
        self._original_predictions = processed_results
        self._original_shapes = [image.shape for image in image_list]

    def convert_batch_predictions(
        self,
        shift_amount_list: Optional[List[List[int]]] = None,
        full_shape_list: Optional[List[List[int]]] = None,
    ):
        """
        Converts batch original predictions to a list of ObjectPrediction objects per image.
        Args:
    ...
    def _create_object_prediction_list_from_original_predictions(
        ...
        for image_ind, image_predictions in enumerate(original_predictions):
            ...
            if hasattr(self, '_original_shapes') and len(self._original_shapes) > image_ind:
                ...
                original_shape = full_shape if full_shape is not None else [640, 640, 3]
